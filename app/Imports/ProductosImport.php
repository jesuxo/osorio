...
class ProductosImport implements ToCollection, WithHeadingRow, WithChunkReading
{
    ...
    {
        $instancias = Sainsta::where('comercial', $this->matchprod)
            ->get();
    ...
    public function collection(Collection $rows)
    {
        ...
        try {
            $comerciales = Sacomercial::where('matchprod', $this->matchprod)->get();

            foreach ($rows as $index => $row) {
                try {
                    // Validar datos mínimos requeridos
                    if (empty($row['codigo']) || empty($row['nombre']) || empty($row['instancia_codigo'])  ) {
                    ...
                    if (!empty($row['instancia_codigo'])) {
                        $instancia = Sainsta::where('codinst', $row['instancia_codigo'])
                            ->where('comercial', $this->matchprod)
                            ->first();
                    }
                    ...
                    $productoExistente = Saprod::where('codprod', substr($row['codigo'], 0, 15))
                        ->where('comercial', $this->comercialId)
                        ->first();

                    if ($productoExistente) {
                        $this->errores[] = "Fila " . ($index + 2) . ": El producto con código {$row['codigo']} ya existe";
                        $this->fallidos++;
                        continue;
                    }

                    // Crear producto para cada comercial del matchprod
                    foreach ($comerciales as $comercial) {
                        $producto             = new Saprod();
                        $producto->codprod    = substr($row['codigo'], 0, 15);
