...
var provList = new List("provList", options).on("updated", function (list) {
...
xhttp.onload = function () {
    var json_records = JSON.parse(this.responseText);
        ...
        Array.from(json_records).forEach(function (element) {
            ...
            provList.sort('id', { order: "desc" });

        });

        provList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">#VZ01</a>`);
}
...
isCount = new DOMParser().parseFromString(
    provList.items.slice(-1)[0]._values.id,
    "text/html"
);
