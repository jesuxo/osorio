    ...
    var json_records = JSON.parse(this.responseText);
        Array.from(json_records).forEach(function (element) {
            ...
            id: `<a href="javascript:void(0);" class="fw-medium link-primary">#TB${element.id}</a>`,
                ...
                multiple   : isMultiple(element.multiple)
...
var idField         = document.getElementById("id-field"),
    ...
    multiplefield   = document.getElementById("account-multiple-field"),
...
var multipleval = new Choices(multiplefield, {
    searchEnabled: false
});
            ...
            btn.addEventListener("click", function (e) {
                ...
                Array.from(itemValues).forEach(function (x) {
                    ...
                    if (selectedid == itemId) {
                        ...
                        if (multipleval) multipleval.destroy();
                        multipleval = new Choices(multiplefield, {
                            searchEnabled: false
                        });
                        ...
                        var val = new DOMParser().parseFromString(x._values.multiple, "text/html");
                        var multipleSelec = val.body.firstElementChild.innerHTML;


                        activoVal.setChoiceByValue(statusSelec);
                        multipleval.setChoiceByValue(multipleSelec);
            ...
            Array.from(editValues).forEach(function (x) {
                ...
                if (selectedid == itemId) {
                    ...
                    x.values({
                        ...
                        multiple     : isMultiple(multiplefield.value),
                    });
...
var multipleInput = new Choices(document.getElementById('idmultiple'), {
    searchEnabled: false,
    shouldSort: false,
});

multipleInput.passedElement.element.addEventListener('change', function (event) {
    var multipleInputValue = event.detail.value;
    tarjetasList.filter(function (data) {
        matchData = new DOMParser().parseFromString(
            data.values().multiple,
            "text/html"
        );
        var multiplev = matchData.body.firstElementChild.innerHTML;
        var multipleFilter = false;

        if (multiplev == "All" || multipleInputValue == "All") {
            multipleFilter = true;
        } else {
            multipleFilter = multiplev == multipleInputValue;
        }
        if (multipleFilter) {
