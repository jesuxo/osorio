{{-- resources/views/saacxcw.blade.php --}}
    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form method="get" name="form1" id="form1" action="/cxc{{(isset($id) and $id > 0)? '/'.$id : ''}}">
                        @csrf
                        <div class="row align-items-end">
                            ...
                            {{-- NUEVO: Filtro por Operador --}}
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="codoper" class="form-label small text-muted mb-1">
                                        <i class="bi bi-tag me-1"></i>Filtrar por Clasificación
                                    </label>
                                    <select name="codoper" id="codoper" class="form-select form-select-sm" onchange="this.form.submit()">
                                        <option value="">Todas las clasificaciones</option>
                                        @if(isset($operadores) && count($operadores) > 0)
                                            @foreach($operadores as $oper)
                                                <option value="{{ $oper->codoper }}" {{ ($filtroCodoper ?? '') == $oper->codoper ? 'selected' : '' }}>
                                                    {{ $oper->codoper }} - {{ $oper->descrip }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <div>
                                        @if(!empty($filtroCodoper))
                                            <a href="/cxc{{(isset($id) and $id > 0)? '/'.$id : ''}}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="col-xl-4">
            <div class="card shadow-sm">
                ...
                <div class="card-body p-0">
                    @php
                        $dataoperc = '';
                        if(!isset($filtroCodoper) or $filtroCodoper == '')
                            $dataoperc = " and c.CodOper NOT IN ('9999', '32', '33', '8888', '9998', '9997')";
                        else
                            $dataoperc = " and c.CodOper= '$filtroCodoper'";
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body p-0">
                        @php
                            ...
                            $movimientosRecientes = \App\Models\Saacxcw::with(['cliente'])
